...
int main(void) {
  ...
  double s = 1.9729;
  int max_iterate = 10000, err_bound = pow(10, -6);

  //Store mean difference in elements for each iteration.
  double meanArray[10000]={0};
  ...
  for (int k=1; k <= max_iterate; k++) {
	...
	//Points forced to zero, no need to include in mean analysis.
	if (distance_sqrd <= r_sqrd)
	  v[row][col] = 0;

	//Add difference between each element in current and previous iteration. Divide by total number of points to get mean difference.
	else{
	  double meanTemp = (1-s)*v[row][col] + (s/4.0)*(v[row-1][col] + v[row+1][col] + v[row][col-1] + v[row][col+1]);
	mean+=fabs(meanTemp-v[row][col]);
	v[row][col]=meanTemp;}
      }
    }
    meanArray[k]=mean/(grid_rows*grid_cols);
  }

 

// outputting average absolute values between values in current and previous iteration.
  ofstream output4 ("absDifferenceMean.dat");
   for (int col=1; col<=2000; col++) {
      output4  << meanArray[col] <<endl;

    }
   //NOTE: Plot with log of y axis.
