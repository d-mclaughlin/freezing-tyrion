...
int main(void) {
  ...
  // std::vector< std::vector< double > > v(grid_rows, std::vector<double>(grid_cols));

  // Boundary conditions
  double top_plate = 2.5f;
  double bottom_plate = -2.5f;
  ...
  double s = 1.9729;
  double max_iterate = 2000, err_bound = pow(10, -6);
  ...
  ofstream output ("NumericalPotential.dat");
