...
/*Creates data for absolute and relative error graphs.
...
void numerical_analytic_comparison(Grid *num){
  ...
  std::cout<<"Matrix Size: "<<row_size<<" X "<<col_size<<std::endl;
  ...
  //Picks analytical data with suitable matrix size to compare with. If no suitable file exists, just picks the 50x50 one. The output will be complete nonsense for this case.
  ...
  switch(row_size)
    ...
    case 501:
      ...
      break;
    default:
      std::cout<<"WARNING: analytical data of size ' "<<row_size<<" ' not found"<<"\n";
     analytical.open("misc/CylinderPotential50.txt");
      break;
    }
  ...
  std::cout<<out/(row_size*col_size)<<std::endl;
